...
export class AppComponent implements OnInit{
  ...
  // booksTmp = JSON.stringify(this.tmp);
  // books = JSON.parse(this.booksTmp);
  ...
  ngOnInit():void{
    // this.displayedBooks=this.filterOnTitle('ver');
  ...
  filterOnTitle(query:string):any[]{
    ...
    const returnArray=this.books.filter(i=>i?.title.toLowerCase().includes(query?.toLowerCase()));
  ...
  logForm():void{
    ...
    this.displayedBooks=this.filterOnTitle(this.searchForm?.value?.title);
  }
}
